 ...
 * @status
 */
public class apii implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "apii.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(apii.class);
	...
	private final String jobName = "apii";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(apii.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_HYDRONORTH_apii = new byte[0];
		static byte[] commonByteArray_HYDRONORTH_apii = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_HYDRONORTH_apii.length) {
					if (length < 1024 && commonByteArray_HYDRONORTH_apii.length == 0) {
						commonByteArray_HYDRONORTH_apii = new byte[1024];
					} else {
						commonByteArray_HYDRONORTH_apii = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_HYDRONORTH_apii, 0, length);
				strReturn = new String(commonByteArray_HYDRONORTH_apii, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_HYDRONORTH_apii, 0, length);
				strReturn = new String(commonByteArray_HYDRONORTH_apii, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_HYDRONORTH_apii) {
	...
	public void tREST_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "DnA9pD_" + subJobPidCounter.getAndIncrement());
...
///////////////////////    			
				...
				if (log.isDebugEnabled()) {
					class BytesLimit65535_tREST_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tREST_1.append("URL" + " = "
									+ "\"https://v2.horusbi.com.br/api/form/data/176.json?flatdata=1&max=1000000&_apitoken=ujPMPlENAsckOKxqQrHRGoLRLOuQQWmHCWyMHoCAKGzclhOoYesvjdvWStsRRHbLpiUqnePzpRq|qTsKKlyGDYGuDBEimilkmrAnXXlZil*XwoFdMChpkTIRX*gaaE*x\"");
				...
				String endpoint_tREST_1 = "https://v2.horusbi.com.br/api/form/data/176.json?flatdata=1&max=1000000&_apitoken=ujPMPlENAsckOKxqQrHRGoLRLOuQQWmHCWyMHoCAKGzclhOoYesvjdvWStsRRHbLpiUqnePzpRq|qTsKKlyGDYGuDBEimilkmrAnXXlZil*XwoFdMChpkTIRX*gaaE*x";
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final apii apiiClass = new apii();

		int exitCode = apiiClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'apii' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_K9S44FpmEe-r5dP3fNz1cQ");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2024-08-14T17:55:42.656527600Z");
		...
		try {
			...
			java.io.InputStream inContext = apii.class.getClassLoader()
					.getResourceAsStream("hydronorth/apii_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = apii.class.getClassLoader()
		...
		log.info("TalendJob: 'apii' - Started.");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : apii");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'apii' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 66506 characters generated by Talend Cloud Data Integration on the 14 de
 * agosto de 2024 14:55:42 BRT
 ************************************************************************************************/